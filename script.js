...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Asegurar que window.people se inicialice también (people.js debería hacerlo, pero por si acaso)
    if (typeof window.people === 'undefined') {
    ...
    const saveTasks = () => {
        ...
        // No necesitamos guardar window.people aquí, people.js lo hace.
    ...
    const importDataFile = document.getElementById('importDataFile'); // El input file real

    exportDataBtn.addEventListener('click', () => {
        const dataToExport = {
            people: window.people || [], // Asegurar que existe, aunque people.js debería inicializarlo
            tasks: window.tasks || []
        };

        const jsonData = JSON.stringify(dataToExport, null, 2); // null, 2 para formatear con indentación
        ...
        reader.onload = (e) => {
            try {
                ...
                if (confirm('¿Estás seguro de que quieres cargar estos datos? Esto reemplazará todos los datos actuales de personas y tareas.')) {
                    ...
                    if (typeof importedData.people !== 'undefined' && typeof importedData.tasks !== 'undefined') {
                        window.people = Array.isArray(importedData.people) ? importedData.people : [];
                        window.tasks = Array.isArray(importedData.tasks) ? importedData.tasks : [];
                        ...
                        alert('El archivo JSON no tiene la estructura esperada (necesita las claves "people" y "tasks").');
                    }
                }
            } catch (error) {
